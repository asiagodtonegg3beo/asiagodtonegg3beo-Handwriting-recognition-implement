...
print('每次訓練將使用100張圖片,執行800次')
for i in range(800): #訓練次數
    batch_xs, batch_ys = MP.batch(NumberOfOneTraing)
    sess.run(train_step, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.5})
    losss = sess.run(cross_entropy,feed_dict={xs: batch_xs, ys: batch_ys,keep_prob: 0.5})
    if i % 10 == 0:
        test_xs, test_ys = MP.batch(100)
        #print(test_xs[0],test_ys[0])
        print('訓練了',str(1000+i*NumberOfOneTraing),'張圖片',",",'準確率:',float(compute_accuracy(test_xs,test_ys))*100-1,'%')
        xla.append(int(i))
        yla.append(float(compute_accuracy(test_xs,test_ys)))
        ylo.append(losss)
...
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.plot(xla, yla, color='green', linewidth=1.0, linestyle='-')
plt.figure()
plt.title('Loss')
plt.xlabel("Epochs")
plt.ylabel("loss")
plt.plot(xla, ylo, color='orange', linewidth=1.0, linestyle='-')
plt.show()
'''
